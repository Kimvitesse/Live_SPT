...
public class FindLocalMaxima {
    ...
    */
   public static SpotCollection FindMax(
      ...
      SpotCollection maxima = new SpotCollection();
      Rectangle roi_rect;
      if (roi!=null) {
      ...
      int yEnd = yRealEnd - n;
      for (int i=roi_rect.x + n/2; i < xEnd; i+=n2) {
         for (int j=roi_rect.y + n/2; j < yEnd; j+=n2) {
            int mi = i;
            int mj = j;
            ...
            if (!stop && (threshold == 0 || 
              ...
              Spot point = new Spot(mi, mj, 0, R, Q);                 // modifier R radius et Q quality ????????????????
              maxima.add(point,frame);                               // frame ??????????????????????????????????????????
         }
      }
